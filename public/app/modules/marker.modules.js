...
app.controller('ModulesCreateCtrl', ['$scope', '$http', '$routeParams', '$route', '$compile', '$location', 'webStorage', function($scope, $http, $routeParams, $route, $compile, $location, webStorage) {
        $scope.working = true;
        $scope.errors = [];

        var storageKey = 'item_create_' + $routeParams.id;

        if (webStorage.get(storageKey)) {
            $scope.item = webStorage.get(storageKey);
        }
        ...
        $scope.$watch('item', function(item){
            webStorage.add(storageKey, item);
        }, true);
